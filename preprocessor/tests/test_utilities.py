...
def test_create_regions_file():
    """
    Regression test: create_regions_file() must flank each region by 200bp on each side, not 400bp. The bug was
    caused by mutating the shared dict entry before writing it, so the write line's "- 200"/"+ 200" was applied
    on top of an already-adjusted value.
    """
    with tempfile.TemporaryDirectory() as td:
        tmp_dir = Path(td)
        positions_vcf = tmp_dir / 'positions.vcf.gz'
        with gzip.open(positions_vcf, mode='wt') as out_f:
            out_f.write('##fileformat=VCFv4.1\n')
            out_f.write('#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\n')
            out_f.write('chr1\t1000\t.\tA\tG\t.\tPASS\tGENEX\n')
            out_f.write('chr1\t1005\t.\tC\tT\t.\tPASS\tGENEX\n')

        regions_file = tmp_dir / 'regions.bed'
        utils.create_regions_file(regions_file, positions_vcf, verbose=1)

        lines = regions_file.read_text().splitlines()
        assert lines == ['chr1\t800\t1205\tGENEX']
